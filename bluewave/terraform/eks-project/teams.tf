  ...
  platform_teams = {
    admin = {
      users = concat(
        ...
        [
          "arn:aws:iam::172059043171:user/admin"
        ]
      )
    }
  }
...
# resource "aws_iam_role_policy_attachment" "ecr_policy_attachment_app_teams" {
#   for_each = module.eks_blueprints.teams[0].application_teams_iam_role_arn

#   role       = split("role/", each.value)[1]
#   policy_arn = "arn:aws:iam::######:policy/microservices-ecr-access"
# }
