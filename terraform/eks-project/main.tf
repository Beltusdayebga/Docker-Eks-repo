...
locals {
  platform_build_role_arn = "arn:aws:iam::209452254299:role/observsmart-multitenant-build"
...
module "eks_blueprints" {
  ...
  managed_node_groups = {
    t3a_large_az_1 = {
      node_group_name = "${var.region}a-managed-ondemand-t3al"
      ...
      subnet_ids      = data.aws_subnet_ids.public_subnets_az_1.ids
      public_ip              = false 
      pre_userdata           = <<-EOT
    ...
    t3a_large_az_2 = {
      node_group_name = "${var.region}b-managed-ondemand-t3al"
      # instance_types  = ["m5.large"]
      # instance_types  = ["t3a.large"]
      instance_types  = ["r6a.large"]
      desired_size    = 1
      ...
      subnet_ids      = data.aws_subnet_ids.public_subnets_az_2.ids
      public_ip              = false 
      pre_userdata           = <<-EOT
                    yum install -y amazon-ssm-agent
                    systemctl enable amazon-ssm-agent && systemctl start amazon-ssm-agent
    ...
    t3a_large_az_3 = {
      node_group_name = "${var.region}c-managed-ondemand-t3al"
      # instance_types  = ["m5.large"]
      # instance_types  = ["t3a.large"]
      instance_types  = ["r6a.large"]
      desired_size    = 1
      ...
      subnet_ids      = data.aws_subnet_ids.public_subnets_az_3.ids
      public_ip              = false 
      pre_userdata           = <<-EOT
                    yum install -y amazon-ssm-agent
                    systemctl enable amazon-ssm-agent && systemctl start amazon-ssm-agent
...
module "eks_blueprints_kubernetes_addons" {
  ...
  #   values = [
  #     "${file("aws_load_balancer_controller_values.yaml")}"
  #     ]
  # }
